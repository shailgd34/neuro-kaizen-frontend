...
export default function DriftTab({ data }: DriftTabProps) {
   ...
   const metadata = [
      ...
      { label: "Current NKPI", value: typeof data.nkpi === 'number' ? data.nkpi.toFixed(2) : (data.nkpi || "0.00") },
   ...
   const domainAnalysis = latestMetric?.domains?.map(d => ({
      domain: d.domain,
      zBase: ((d.baseline ?? 0) / 25).toFixed(2),
      zRoll: ((d.score ?? 0) / 25).toFixed(2),
      zCombo: (d.zScore ?? 0).toFixed(2),
      volatility: (Math.abs(d.delta ?? 0) / 10).toFixed(2),
      classification: (d.drift || 'STABLE').toUpperCase()
   })) || [];
                     ...
                     <tbody>
                        {coreMetrics.map((m, idx) => (
                           <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.01]">
                              ...
                              <td className="py-4 text-center font-bold text-white tabular-nums">{(m.value ?? 0).toFixed(2)}</td>
                              <td className="py-4 text-center text-gray-500 font-mono italic tabular-nums">{(m.threshold ?? 0).toFixed(2)}</td>
                              <td className={`py-4 text-right px-2 font-black italic ${m.status === 'NORMAL' ? 'text-emerald-400' : 'text-amber-500'
                                 }`}>
                                 {m.status}
                              </td>
                           </tr>
                     ...
                     <tbody>
                        {varianceParams.map((p, idx) => (
                           <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.01]">
                              ...
                              <td className="py-4 text-center font-bold text-white tabular-nums">{(p.value ?? 0).toFixed(2)}</td>
                  ...
                  <tbody>
                     {domainAnalysis.map((row, idx) => (
                        <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors group">
                           ...
                           <td className={`py-5 text-right px-4 font-black italic text-xs ${row.classification === 'STABLE' || row.classification === 'NORMAL' ? 'text-emerald-400/80' :
               ...
               <div className="flex flex-col gap-4">
                  {[
                     ...
                     <div key={idx} className="flex gap-5 items-center p-5 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-white/[0.04] transition-colors">
                        <div className={`w-2 h-2 shrink-0 rounded-full shadow-[0_0_8px] ${rule.color === 'bg-amber-500' ? 'shadow-amber-500' :
